...
if(item.onload){await(item.onload)()}else if(window[item.hasAttribute("onload")]){await(window[item.getAttribute("onload")])()}}
new ComponentResolver(item,extra);new ConditionResolver(item,extra);if(!item.hasAttribute("@foreach")){new VariableResolver(item,[])}else{new ForeachResolver(item,extra)}};const ForeachResolver=function(item,extra){if(item.hasAttribute("@foreach")){let targetName=item.getAttribute("@foreach");let last=item;let tmp=item.data[targetName];if(item.hasAttribute("@sortby")){let sort=item.getAttribute("@sortby");tmp=item.data[targetName];tmp.sort(sortBy(sort))}
for(var key in tmp){if(!tmp.hasOwnProperty(key))continue;let value=tmp[key];let clone=item.cloneNode();clone.innerHTML=item.innerHTML;new ComponentResolver(clone,extra);new ConditionResolver(clone,extra);if(!clone.data){clone.data={}}
clone.data["@foreach"]={value:value,key:key}
new VariableResolver(clone,["@foreach"]);insertAfter(clone,last);last=clone}
item.parentNode.removeChild(item)}};const ConditionResolver=function(item){const IF=0,ELSE=1,ELSEIF=2;this.result=!1;const ID=ConditionResolver.stack.length;const PREV=ID-1<0?null:ID-1;if(item.hasAttribute("@if")){let result="";let statement=item.getAttribute("@if");if(statement.trim()!==""){try{result=eval(statement);if(result){}else{item.parentNode.removeChild(item)}
...
for(let c in Components){if(c.toLowerCase()===key.toLowerCase()){try{item.data=extra.bindElement.data;let tmp=Components[c];(tmp).call(item);break}catch(e){console.error(e);break}}}};const VariableResolver=function(item,path=[]){const REGEX=/@[A-z0-9\.]*/g;const SUCCESS=0,NO_DATA=1,NO_MATCH=2;let resolve=function(input,callback){let matches=[...new Set(input.matchAll(REGEX))];if(matches.length===0){(callback)(undefined,NO_MATCH);return}
matches.forEach(match=>{let key=match[0].substr(1);let data=item.data;if(data){path.forEach(function(location){if(data[location]){data=data[location]}else{console.warn("Variable \""+(path.join("->"))+"->"+key+"\" doesn't exist in the data object.",item);return}});let result=new Function("return this."+key+";").call(data);if(!isElement(result)){(callback)(input.replace(new RegExp(match),result),SUCCESS,!1)}else{(callback)(result,SUCCESS,!0)}}else{(callback)(undefined,NO_DATA,!1)}})};if(item.children.length===0){resolve(item.innerHTML,function(result,state,isElement){if(state===SUCCESS){if(!isElement){item.innerHTML=result}else{item.innerHTML="";item.appendChild(result)}}})};let attributes=[...item.attributes];attributes.forEach(attribute=>{resolve(attribute.value,function(result,state){if(state===SUCCESS){item.setAttribute(attribute.name,result)}})})};const recursiveParser=async function(target,allowVariables,extra={}){const tmp=new Array();await foreach(target.children,child=>{tmp.push(child)});await foreach(tmp,async child=>{switch(child.tagName){case "SCRIPT":if(child.hasAttribute("src")){await use.js("@"+child.getAttribute("src"))}else{await eval(child.innerText)}
