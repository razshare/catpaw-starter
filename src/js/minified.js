...
function Book(content=null,options={}){
    let $this = create("div",content,options);

    let update=function(){
        if(busy) return;
        let widths = new Array();
        for(let i=0;i < $this.children.length; i++){
            widths.push(Percent(100));
        }
        $this.style.gridTemplateColumns=widths.join(" ");
    };$this.update=update;
    ...
    let hook = function(deltaX,time){
        const unit = $this.children[0].offsetWidth*0.2;
        const velocity = (deltaX/time).truncate(1);
        if(velocity <= -0.5){
            page(currentPage-1);
        }else if(velocity >= 0.5){
            page(currentPage+1);
        }else if(deltaX <= unit && deltaX >= -unit){
            page(currentPage);
        }else if(deltaX < 0 ){
            page(currentPage-1);
        }else if(deltaX > 0){
    ...
    $this.addEventListener("touchend",e=>{
        ...
        last = null;
        try{
            hook(start.clientX-end.clientX,end.time-start.time);
            start = null;
            end = null;
        }catch(e){}
    });
    ...
    $this.addEventListener("touchmove",e=>{
        if(busy) return;
        if(last !== null){
            const x = e.touches[0].clientX - last.clientX;
            $this.scrollLeft += -x;
        }
        last = e.touches[0];
        end = e.touches[0];
    ...
    let page=function(i,inc=15){
        ...
        return new Promise(resolve=>{
            ...
            let destination = $this.children[i].offsetLeft;
            let x = $this.scrollLeft;
            let deltaX = destination - x;
            (async function poll(){
                if(deltaX > 0 && x < destination){
                    x +=inc;
                    if(x > destination) x = destination;
                    $this.scrollLeft=x;
                } else if(deltaX < 0 && x > destination){
                    x -=inc;
                    if(x < destination) x = destination;
                    $this.scrollLeft=x;
                }else {
